...
int main(void)
{
    ...
    while (lives > 0 && bricks > 0)
    {
        ...
        if(object!=NULL)
        {
        if (strcmp(getType(object),"GRect\0")==0)
...
void initBricks(GWindow window)
{
    ...
    for(i=0;i<ROWS;++i)
    for(j=0;j<COLS;++j)
    {double x=(j+1)*5+(j*bwidth);
     double y=(i+1)*5+(i*bheight);
...
{
    // TODO
    GOval ball=newGOval(200, 300, 2*RADIUS, 2*RADIUS);
...
{
    // TODO
    GRect paddle = newGRect(200, 590, pwidth, pheight);
